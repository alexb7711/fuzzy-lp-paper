...
triangular fuzzy numbers for the slack variables are valid. [[eq:fflp-example-crisp]] is now said to be a crisp
representation of [[eq:fflp-example]] in standard form. Solving the FFLP utilizing the Octave LP module (using both the
Nasseri and Kumar methods to verify the results), the example problem has a solution as displayed in
[[tab:fflp-example-solution]].
