...
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\include{sup-doc/milp-pap-paper-frontiers/tex/preamble.tex}
...
\label{sec:org09485ac}
\section{Preliminaries}
\label{sec:org8f75af5}
\subsection{Mixed Integer Linear Program}
\label{sec:org5b75b43}
...
\label{sec:org8be8c40}
The following is a FFLP model to the continuous dynamical BAP that is able to allocate a quay to an incoming vessel
(Marilyn Bello and Gonzalo N{\'a}poles and Ivett Fuentes and Isel Grau and Rafael Falcon and Rafael Bello and Koen Vanhoof, 2019). The model assumes that the arrival time, \(\tilde{a}\), berthing time, \(\tilde{u}\), and
...
notation of (Marilyn Bello and Gonzalo N{\'a}poles and Ivett Fuentes and Isel Grau and Rafael Falcon and Rafael Bello and Koen Vanhoof, 2019) while attempting to remain as consistent as possible with the notation
...
\label{sec:org8e118f9}
To derive the queuing constraints, begin by rewriting the set of crisp constraints that are of interest as fuzzy
variables as shown in \ref{eq:fuzzy-constrs-pre}. \ref{subeq:fp_m_time} fuzzifies all the temporal terms; however note
that the binary decision variable, \(\sigma_{ij}\), is now written as \(\sigma^k_{ij}\), where \(k \in \{1,..,|\tilde{u}_i|\} \in \mathbb{Z}\) is
used to indicate the temporal binary decision for the \(k^{\text{th}}\) fuzzy value in \(\tilde{u}_i\).
\ref{subeq:fp_m_valid_pos} and \ref{subeq:fp_m_sigma} duplicate the constraints \(|\tilde{u}_i|\) times. Using triangular
fuzzy numbers, that would imply constraints for the lower, middle, and upper bounds of the temporal impreciseness.
Similarly, \ref{subeq:fp_m_detach}-\ref{subeq:fp_m_valid_depart} are equivalent to their crisp counterparts, but with
fuzzified terms. \ref{subeq:fp_gpgret}-\ref{subeq:fp_gwles} fuzzify the linearization terms in a similar manner, but the
crisp spatial term, \(w_{iq}\) is included. This is allowed because although the time may be allowed to fluctuate, the
specified queue is the same for each element of the fuzzy temporal values. Thus, when the fuzzy constraints are
converted into crisp constraints, the constraint validity is maintained. \ref{subeq:fp_binaryspace} simply updates the
binary decision term.

\begin{multicols}{2}
\begin{subequations}
                                                     \label{eq:fuzzy-constrs-pre}
\begin{equation}
    \tilde{u}_i - \tilde{u}_j - \tilde{s}_j - (\sigma^k_{ij} - 1)T \geq 0 \label{subeq:fp_m_time}         \\
\end{equation}
\begin{equation}
    \sigma^k_{ij} + \sigma^k_{ji} + \psi_{ij} + \psi_{ji} \geq 1                     \label{subeq:fp_m_valid_pos}    \\
\end{equation}
\begin{equation}
    \sigma^k_{ij} + \sigma^k_{ji} \leq 1                                       \label{subeq:fp_m_sigma}        \\
\end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
   \sigma^k_{ij}, \psi_{ij} \in \{0,1\}\;                                   \label{subeq:fp_binaryspace}        \\
\end{equation}
\end{subequations}
\end{multicols}
