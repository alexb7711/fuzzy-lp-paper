...
\ref{fig:characteristic-plot} demonstrates a membership function of a triangular fuzzy number. For a formal definition,
consider the \ref{def:membership-function}:
...
property that an \(L\) or \(R\) function must have.
...
The definition of a LR fuzzy number may now be developed given the basis of what properties an \(L\) (or \(R\)) function
...
(\ref{fig:characteristic-plot}). The triangular fuzzy numbers shall also be defined over the set of real numbers \(\mathbb{R}\). Consider
...
if its membership function is defined as \ref{eq:triangular-fuzzy-number}. \ref{fig:characteristic-plot} depicts a visual
